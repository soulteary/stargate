...
- **TOTPEnrollRoute / TOTPEnrollConfirmAPI**: TOTP 绑定页与确认（通过 Herald TOTP）
- **TOTPRevokeRoute / TOTPRevokeConfirmAPI**: TOTP 解绑页与确认（通过 Herald TOTP）
- **LogoutRoute**: 登出处理
...
密码算法由外部包 [secure-kit](https://github.com/soulteary/secure-kit) 提供。若要新增算法：

1. 实现 `HashResolver` 接口（可在 secure-kit 或本地适配器中实现）
2. 在 `internal/config/validation.go` 的 `SupportedAlgorithms` 映射中注册该算法
3. 确保配置校验器在 `PASSWORDS` 格式中接受新算法名
...
- 会话存储在内存或 Redis（当 `SESSION_STORAGE_ENABLED=true` 时）中，访问快速
...
- 多阶段构建，减小镜像体积
- 构建阶段：`golang:1.26-alpine3.22`；运行阶段：`alpine:3.22`（含 curl 用于健康检查）
...
- 测试覆盖的模块：
  ...
  - 密码校验与算法（通过 secure-kit；在 `internal/auth/auth_test.go`、`internal/config/config_test.go` 中测试）
