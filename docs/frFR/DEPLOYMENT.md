...
- **Limitation de débit entre répliques :** Les compteurs de limitation sont partagés via la même connexion Redis. Sans `SESSION_STORAGE_ENABLED=true`, chaque réplique applique son propre quota et N répliques autorisent jusqu'à N fois `RATE_LIMIT_LOGIN_MAX` et `RATE_LIMIT_VERIFICATION_MAX`. Les quotas sont comptés par adresse client : le reverse proxy doit figurer dans `TRUSTED_PROXIES`, sinon chaque client est attribué au proxy et partage un seul quota.
