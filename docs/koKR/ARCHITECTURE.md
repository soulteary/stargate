...
- **터미널 출력**: [Pterm](https://github.com/pterm/pterm)
...
- **SendVerifyCodeAPI**: 인증 코드 발송(Herald로 챌린지 생성)
- **TOTPEnrollRoute / TOTPEnrollConfirmAPI**: TOTP 바인드·확인(Herald TOTP 경유)
...
알고리즘은 외부 패키지 [secure-kit](https://github.com/soulteary/secure-kit)에서 제공됩니다. 추가 시:
1. `HashResolver` 인터페이스 구현(secure-kit 또는 로컬 어댑터)
2. `internal/config/validation.go`의 `SupportedAlgorithms` 맵에 등록
3. 설정 검증에서 `PASSWORDS` 형식의 새 알고리즘 이름 허용
...
- 세션은 메모리 또는 Redis(`SESSION_STORAGE_ENABLED=true` 시)에 저장, 빠른 액세스
...
- 빌드 단계: `golang:1.26-alpine3.22`; 실행 단계: `alpine:3.22`(헬스 체크용 curl 포함)
...
- 테스트 커버리지에는 다음이 포함됩니다:
  ...
  - 비밀번호 검증 및 설정(`internal/auth/auth_test.go`, `internal/config/config_test.go`); 알고리즘은 secure-kit
