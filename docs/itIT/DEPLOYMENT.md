...
- **Limitazione tra repliche:** I contatori di limitazione sono condivisi attraverso la stessa connessione Redis. Senza `SESSION_STORAGE_ENABLED=true` ogni replica applica la propria quota e N repliche consentono fino a N volte `RATE_LIMIT_LOGIN_MAX` e `RATE_LIMIT_VERIFICATION_MAX`. Le quote sono calcolate per indirizzo client: il reverse proxy deve comparire in `TRUSTED_PROXIES`, altrimenti ogni client viene attribuito al proxy e condivide una sola quota.
