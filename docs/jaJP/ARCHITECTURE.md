...
- **ターミナル出力**: [Pterm](https://github.com/pterm/pterm)
...
- **SendVerifyCodeAPI**: 認証コード送信（Herald でチャレンジ作成）
- **TOTPEnrollRoute / TOTPEnrollConfirmAPI**: TOTP バインド画面・確認（Herald TOTP 経由）
...
パスワード検証は `internal/auth` と `internal/config` で行われます。本番環境では `bcrypt`、ローカルテストでのみ `plaintext` を使用でき、MD5 とソルトなし SHA-512 は拒否されます。セッションと認証状態は session-kit により提供されます。
...
- 本番環境では `bcrypt`、ローカルテストでのみ `plaintext` をサポート
- パスワード設定は環境変数経由で渡され、コードに保存されない
- パスワードは大文字と小文字を区別し、空白も変更せずに検証
...
アルゴリズムは外部パッケージ [secure-kit](https://github.com/soulteary/secure-kit) で提供されます。追加する場合：
1. `HashResolver` インターフェースを実装（secure-kit またはローカルアダプタ）
2. `internal/config/validation.go` の `SupportedAlgorithms` マップに登録
3. 設定検証で `PASSWORDS` 形式の新アルゴリズム名を許可する
...
- セッションはメモリまたは Redis（`SESSION_STORAGE_ENABLED=true` 時）に保存、高速アクセス
...
- テストカバレッジには以下が含まれます：
  ...
  - パスワード検証と設定（`internal/auth/auth_test.go`、`internal/config/config_test.go`）；アルゴリズムは secure-kit
