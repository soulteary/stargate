...
```bash
docker run -e AUTH_HOST=auth.example.com -e PASSWORDS=plaintext:yourpassword ghcr.io/soulteary/stargate:v1.2.0
```
...
- 利用可能な場合は認証済みユーザー ID が値になり、パスワードのみの認証では `authenticated` を使用します
...
サービスのリスニングポート（ローカル開発のみ）。config パッケージで他の環境変数と同様に読み込み・検証されます。

| 属性 | 値 |
|------|-----|
| **型** | String |
| **必須** | いいえ |
| **デフォルト** | 空（空の場合はサーバーはデフォルトポート `:80` を使用） |
