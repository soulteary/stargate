...
Stargate 是一个轻量级前向认证服务，用于在反向代理边界集中处理认证。它可与 Traefik 集成，并为受保护服务提供密码、会话、Warden 和 Herald 认证流程。
...
- **集成认证**：与 Warden（用户白名单）和 Herald（OTP/验证码）集成
...
- **部署可观测性**：提供健康检查、指标、结构化日志和容器安全默认值

### 📦 开发体验
- **Docker 优先**：开箱即用的完整 Docker 镜像和 docker-compose 配置
- **Traefik 集成**：通过明确的 ForwardAuth 中间件标签和响应头配置接入
...
- 🔄 **[v1.0.0 迁移指南](docs/zhCN/MIGRATION_V1.md)** - 从 v0.12.0 升级时的配置与行为变化
