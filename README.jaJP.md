...
Stargate は、リバースプロキシ境界で認証を集中管理する軽量な Forward Auth サービスです。Traefik と連携し、保護対象サービスにパスワード、セッション、Warden、Herald の認証フローを提供します。
...
- **統合認証**：Warden（ユーザーホワイトリスト）と Herald（OTP/検証コード）との統合

## ✨ 機能

### 🔐 セキュリティ制御

- **パスワード検証**：本番環境では bcrypt、ローカルテストでのみ plaintext を使用
...
- **デプロイ可観測性**：ヘルスプローブ、メトリクス、構造化ログ、強化されたコンテナ既定値
...
ローカル開発では、Go 1.27+ がインストールされていることを確認し、次を実行します：
...
- 🔄 **[v1.0.0 移行ガイド](docs/enUS/MIGRATION_V1.md)** - v0.12.0 以降の設定と動作の変更
...
**サポート対象：** `plaintext`（ローカルテストのみ）と `bcrypt`。MD5 とソルトなし SHA-512 は拒否されます
...
- ✅ 本番環境では `bcrypt` を使用し、`plaintext` はローカルテストに限定
