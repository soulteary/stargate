...
# Once a short sudo option takes an attached operand, letters inside that
# operand are not additional option flags (for example, the trailing t in root).
printf '%s\n' \
  '' \
  '```bash' \
  'sudo -uroot htpasswd -bn "" password' \
  '```' \
  >> "$temp_dir/docs/enUS/CONFIG.md"
if (cd "$temp_dir" && bash .github/scripts/check-doc-contracts.sh "$base_sha" >/dev/null 2>&1); then
  echo "Expected an unsafe command after an attached sudo user failure" >&2
  exit 1
fi
git -C "$temp_dir" checkout -q -- docs/enUS/CONFIG.md
...
printf '%s\n' \
  '' \
  '```bash' \
  'sudo -uhtpasswd printf "%s\\n" "-b"' \
  '```' \
  >> "$temp_dir/docs/enUS/CONFIG.md"
if ! (cd "$temp_dir" && bash .github/scripts/check-doc-contracts.sh "$base_sha" >/dev/null 2>&1); then
  echo "Expected an attached sudo operand named htpasswd to pass" >&2
  exit 1
fi
git -C "$temp_dir" checkout -q -- docs/enUS/CONFIG.md
...
# The first operand-taking flag terminates an xargs short-option bundle. Any
# remaining characters are its attached operand, not later flags.
for attached_xargs_command in \
  "printf 'passwordn' | xargs -dn htpasswd -bn \"\"" \
  "printf password | xargs -Iitem htpasswd -bn \"\" item"
do
  printf '%s\n' \
    '' \
    '```bash' \
    "$attached_xargs_command" \
    '```' \
    >> "$temp_dir/docs/enUS/CONFIG.md"
  if (cd "$temp_dir" && bash .github/scripts/check-doc-contracts.sh "$base_sha" >/dev/null 2>&1); then
    echo "Expected an unsafe command after an attached xargs operand failure" >&2
    exit 1
  fi
  git -C "$temp_dir" checkout -q -- docs/enUS/CONFIG.md
done
...
printf '%s\n' \
  '' \
  '```bash' \
  'printf password | xargs -d htpasswd printf "%s\\n" "-b"' \
  '```' \
  >> "$temp_dir/docs/enUS/CONFIG.md"
if ! (cd "$temp_dir" && bash .github/scripts/check-doc-contracts.sh "$base_sha" >/dev/null 2>&1); then
  echo "Expected a separate xargs operand named htpasswd to pass" >&2
  exit 1
fi
git -C "$temp_dir" checkout -q -- docs/enUS/CONFIG.md
...
# Attached timeout option operands must not consume the separate duration or
# the delegated command which follows it.
for attached_timeout_command in \
  'timeout -k1s 2s htpasswd -bn "" password' \
  'timeout -sTERM 2s htpasswd -bn "" password'
do
  printf '%s\n' \
    '' \
    '```bash' \
    "$attached_timeout_command" \
    '```' \
    >> "$temp_dir/docs/enUS/CONFIG.md"
  if (cd "$temp_dir" && bash .github/scripts/check-doc-contracts.sh "$base_sha" >/dev/null 2>&1); then
    echo "Expected an unsafe command after an attached timeout operand failure" >&2
    exit 1
  fi
  git -C "$temp_dir" checkout -q -- docs/enUS/CONFIG.md
done
...
printf '%s\n' \
  '' \
  '```bash' \
  'timeout -k htpasswd 2s printf "%s\\n" "-b"' \
  '```' \
  >> "$temp_dir/docs/enUS/CONFIG.md"
if ! (cd "$temp_dir" && bash .github/scripts/check-doc-contracts.sh "$base_sha" >/dev/null 2>&1); then
  echo "Expected a separate timeout operand named htpasswd to pass" >&2
  exit 1
fi
git -C "$temp_dir" checkout -q -- docs/enUS/CONFIG.md
