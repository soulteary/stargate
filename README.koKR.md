...
Stargate는 리버스 프록시 경계에서 인증을 중앙화하는 경량 Forward Auth 서비스입니다. Traefik과 통합되어 보호 대상 서비스에 비밀번호, 세션, Warden 및 Herald 인증 흐름을 제공합니다.
...
- **통합 인증**: Warden(사용자 화이트리스트) 및 Herald(OTP/인증 코드) 연동
...
- **배포 관측성**: 상태 프로브, 메트릭, 구조화 로그 및 강화된 컨테이너 기본값
...
- 🔄 **[v1.0.0 마이그레이션 가이드](docs/enUS/MIGRATION_V1.md)** - v0.12.0 이후 구성 및 동작 변경
