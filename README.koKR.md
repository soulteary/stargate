...
- **API 엔드포인트**: `GET /_auth`(인증 확인), `GET /_login`(로그인 페이지), `POST /_login`(로그인), `POST /_send_verify_code`(OTP 발송), `GET /_logout`(로그아웃), `GET /_session_exchange`(크로스 도메인), `GET /totp/enroll`, `POST /totp/enroll/confirm`, `GET /totp/revoke`, `POST /totp/revoke`(Herald TOTP 사용 시), `GET /health`(상태 확인), `GET /metrics`(Prometheus)
